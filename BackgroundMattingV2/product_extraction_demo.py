...
video_path="BackgroundMattingV2/outputs/processed_video.mp4"
...
cap = cv2.VideoCapture("BackgroundMattingV2/inputs/rsrc.mp4")
fps = cap.get(cv2.CAP_PROP_FPS)
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # Codec for .mp4 format
video_writer = cv2.VideoWriter("BackgroundMattingV2/outputs/boxed_product.mp4", fourcc, fps, (frame_width, frame_height))
...
for frame_name in frame_names:
    ...
    _, image = load_image(local_image_path)
    ...
    if class_ob not in phrases:#len(boxes) == 0
        ...
        annotated_frame=image_source # BGR
    else:
        ...
        annotated_frame = annotate(image_source=image_source[...,::-1], boxes=sorted_boxes, logits=sorted_logits, phrases=sorted_phrases)
